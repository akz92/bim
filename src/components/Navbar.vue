<template>
  <div id="navbar">
    ...
    <i id="navbar-reload" class="nav-icons" title="Reload page" v-bind:class="{ disabled: !hasActiveIndex }" @click="toggleReload">
...
export default {
  ...
  computed: {
    ...
    hasActiveIndex: function () { return typeof this.activeIndex === 'number' }
  },
  methods: {
    ...mapActions([
      'addTab',
    ...
    submit: function () {
      ...
      if (this.hasActiveIndex) {
        this.setUrl({ url, index: this.activeIndex, updateWebview: true })
      } else {
        this.addTab({ url })
      }
    }
